...
func (c *Client) req(method, path string, body io.Reader, intercept func(*http.Request)) (rs *http.Response, err error) {
	...
	for { // TODO auth.continue() strategy(true|n times|until)?
		...
		if redo {
			...
			// 【新增安全检查】
			// 如果 Body 无法重置（比如它是 Pipe），绝对不要尝试 ReadAll！
			// 直接报错退出，或者检查是否能无损 GetBody
			if r.GetBody == nil && body != nil {
				// 检查 body 是否是 http.NoBody 或者 nil
				if _, ok := body.(*bytes.Buffer); !ok && body != http.NoBody {
					// 这是一个流，且没有 GetBody 方法，由于我们无法倒带，
					// 强行 continue 会导致 GetBody() 尝试把流读进内存，或者后续发空包。
					// 既然预认证失败了，这里直接报错比爆内存好。
					return nil, fmt.Errorf("critical error: authentication required retry but stream is not seekable. pre-auth failed")
				}
			}
...
func (c *Client) put(path string, stream io.Reader, contentLength int64) (status int, err error) {
	rs, err := c.req("PUT", path, stream, func(r *http.Request) {
		...
		// 【关键修复】显式禁止 GetBody
		// 告诉 Go 标准库："这个流是一次性的，绝对不要尝试快照、缓冲或重试它"
		r.GetBody = nil
