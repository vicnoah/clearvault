...
func (c *Client) WriteStreamWithLength(path string, stream io.Reader, contentLength int64, _ os.FileMode) (err error) {
	// 1. 创建父目录 (这通常也能起到预认证的作用，但为了保险，我们下面会显式处理)
	err = c.createParentCollection(path)
	if err != nil {
		return err
	}
	...
	// =================================================================
	// 2. 【核心修复】预热认证 (Warm-up / Probe)
	// 我们先发一个 HEAD 请求。
	// 目的：
	//   - 如果服务器需要 Auth，这个轻量请求会触发 401->Retry 流程。
	//   - 因为 HEAD 请求没有 Body，Retry 没有任何内存代价。
	//   - 成功后，c.auth 内部会缓存 Session/Nonce。
	// =================================================================

	// 注意：即使文件不存在(404)，服务器通常也会先校验权限，所以也能拿到 Nonce
	_, _ = c.req("HEAD", path, nil, nil)

	// 3. 正式发送大文件
	// 此时 c.auth 应该已经持有有效的认证信息。
	// 当 c.put 发起请求时，它会直接带上正确的 Authorization 头，
	// 服务器直接返回 201 Created，不再返回 401，从而避免了“重读 Pipe”的灾难。
	s, err := c.put(path, stream, contentLength)
	if err != nil {
		return err
	}
